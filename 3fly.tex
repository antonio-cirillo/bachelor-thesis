...
\end{figure}
La prima fase prevede l'inizializzazione degli ambienti di esecuzione dichiarati all'interno del codice Fly, provedendo ad effettuare il login sui diversi Cloud Provider utilizzati, tramite le credenziali di accesso specificate all'interno del codice Fly, e l'istanziazione dei servizi necessari. Nel capitolo precedente abbiamo introdotto il concetto di funzione Fly. Nella seconda fase di esecuzione di un programma Fly, tutte le funzioni Fly dichiarate all'interno del codice vengono tradotte in linguaggio JavaScript o Python, in base ad una scelta effettuata dallo sviluppatore previa esecuzione, e successivamente caricate sul rispettivo ambiente Cloud. Una volta terminate queste due fasi di inizializzazione, il programma principale viene mandato in esecuzione. Al suo termine vengono eseguite le operazioni necessarie per eliminare tutte le istanze dei servizi Cloud allocati in precedenza.
...
Nel Listato~\ref{lst:pigreco} possiamo vedere come il programma sia composto da due funzioni, \verb|hit| ed \verb|estimation|. La funzione \verb|hit| genera un punto nel quadrato con coordinate x ed y casuali. Successivamente la funzione controlla se il punto generato appartine o meno all'area del cerchio inscritto nel quadrato utilizzando il teorema di Pitagora: se l'ipotenusa è minore di 1 il punto generato casualmente appartiene all'area del cerchio. Successivamente questa informazione viene inviata sul canale \verb|ch|. La funzione \verb|hit| viene eseguita un numero di volte pari alla quantità di punti che si intende generare. In questo caso la funzione viene eseguita per un totale di 100 volte, generando quindi 100 punti all'interno del quadrato. Al termine delle 100 esecuzioni viene eseguita la funzione \verb|estimation| che si occupa di effettuare la stima del PI Greco, utilizzando la formula \ref{eq:1}. Calcolata la stima, quest'ultima viene stampata a schermo tramite una \verb|println|. \\
...
\end{itemize}
Gli ambienti di esecuzioni permettono allo sviluppatore di scegliere su quale ambiente eseguire le funzioni Fly. È importante ricordare che le funzioni Fly per essere eseguite su ambienti Cloud devono essere tradotte in un linguaggio supportato dal Cloud Provider, per questo motivo è richiesto all'utente di specificare il linguaggio di programmazione in cui verranno esse verranno tradotte. Nel listato~\ref{lst:pigreco} le prime 3 righe del codice, riportate a seguire, contengono la dichiarazione dei due ambienti di esecuzione utilizzati: ambiente locale e ambiente AWS.
...
\begin{itemize}
    ...
    \item file \textit{pom.xml} per la gestione del progetto e specifica delle librerie necessarie al programma;
    \item le varie dipendenze dichiarate all'interno del file POM.
\end{itemize}
