...
Il motivo principale per il quale questo paradigma si è diffuso molto rapidamente è il modello di costo su cui si basa il Cloud Computing, ovvero il modello pay-as-you-go. Questo modello permette alle aziende di pagare soltanto per i servizi utilizzati, in relazione al tempo di impiego. Grazie al Cloud Computing le aziende non hanno bisogno di possedere un'infrastruttura di calcolo ma possono noleggiarla dal Cloud Provider che, inoltre, si occupa anche della sua manutenzione, gestione e aggiornamento. Questo vantaggio non va solo a favore di quelle che sono le grandi imprese preesistenti, ma favorisce anche l'ingresso nel mercato di nuove imprese.
...
\begin{description}
    ...
    \item[Private Cloud]Sistema di server dedicato completamente alle infrastrutture di una singola azienda. Offre una maggiore sicurezza rispetto al Public Cloud ma è svantaggioso sotto il punto di vista dei costi per l'azienda e non ha la scalabilità quasi illimitata fornita dai grandi Cloud Provider.
...
\begin{description}
    ...
    \item[Software-as-a-Service (SaaS)]Con il modello SaaS il consumatore ha la possibilità di utilizzare software pronti all'uso, eseguiti e gestiti dal Cloud Provider, tramite l'utilizzo del browser. Un esempio comune di applicazione SaaS è Gmail che permette all'utente di inviare e ricevere e-mail tramite un qualsiasi browser, trascurando ogni aspetto dell'infrastruttura sottostante.
\end{description}
...
\begin{itemize}
    ...
    \item \textit{flessibilità} - possibilità di poter rappresentare i dati utilizzando diversi modelli di database;
    \item \textit{elevate prestazioni} - ottimizzati per modelli di dati specifici e schemi di accesso che consentono prestazioni più elevate rispetto ai database relazionali;
    \item \textit{scalabilità} - le strutture dati utilizzate permettono di avere una scalabilità orizzontale invece che verticale.
...
\begin{itemize}
    \item \textit{completamente automatizzato} - il servizio di database offerto è completamente gestito da Atlas \cite{atlas}. Esso si occupa di automatizzare operazioni come il provisioning, la configurazione e la distribuzione dell'infrastruttura. Grazie a queste funzionalità, Atlas permette di utilizzare in modo semplice e intuitivo un database NoSQL su Cloud, in modo da concentrarsi solo su quello che è la memorizzazione e la gestione dei dati, ignorando tutta la gestione dell'infrastruttura sottostante;
    \item \textit{compatibile} - completamente compatibile con i vari servizi di gestione di database documentali offerti dai vari Cloud Provider;
    ...
    \item \textit{affidabile} - tolleranza di errore distribuita e ripristino automatizzato dei dati. I cluster sono progettati per la resilienza con un set di repliche a tre nodi minimo distribuiti tra le zone di disponibilità all'interno di un'area cloud. Nel caso in cui un nodo primario non è disponibile, Atlas eseguirà automaticamente un rimpiazzo di quest'ultimo con una componente analoga in pochi secondi, senza necessità di intervento manuale.
\end{itemize}

\subsection{Database su Cloud}
I Cloud Provider forniscono un insieme di servizi, racchiusi all'interno del modello \textbf{Database-as-Service (DBaaS)}, che consentono la creazione di database, funzionalità di backup e sistemi di sicurezza integrata. I servizi DBaaS permettono al consumatore di utilizzare il database senza preoccuparsi di tutte le operazioni di gestione. Tramite questo modello, il database viene eseguito sull'infrastruttura del service provider, includendo l'automazione nelle aree di provisioning, backup, ridimensionamento, disponibilità elevata, sicurezza, applicazione di patch e monitoraggio dello stato. \\
I servizi DBaaS offrono diversi vantaggi:
\begin{itemize}
    \item \textit{scalabilità} - grazie al ridimensionamento dinamico è sempre possibile effettuare un aumento di risorse per i periodi di maggiore attività e una riduzione delle risorse in caso di riduzione della domanda;
    \item \textit{distribuzione in pochi minuti} - l'accesso ai database può essere disponibile in pochi minuti;
    \item \textit{agilità} - le nuove idee di business possono essere testate, convalidate e rese operative in modo semplice e veloce;
    \item \textit{rischi ridotti} - l'automazione delle funzionalità di gestione dei database riduce nettamente le possibilità di downtime del software;
    \item \textit{sicurezza} - i dati di archiviazione Cloud sono crittografati ed accessibili solo agli utenti autorizzati.
\end{itemize}