...
\begin{lstlisting}[language=FLY,caption={File log4j.proprietes generato in automatico da Fly.}, label={lst:log}]
...
\end{lstlisting}  
\begin{lstlisting}[language=FLY,caption={Dichiarazione di entità nosql per la connessione ad un database distribuito su Azure tramite l'utilizzo dei parametri resourceGroup e instance.}, label={lst:nosql:2}]
...
\begin{itemize}
    \item \textbf{query\textunderscore type}: stringa rappresentante il tipo della query da eseguire (\verb|select|, \verb|insert|, \verb|update|, \verb|delete|);
